...
"""Taken from https://raw.githubusercontent.com/tensorflow/tensor2tensor/master/tensor2tensor/utils/bleu_hook.py """
...
#  labels = tf.squeeze(labels, axis=[-1, -2])
  ...
  # blue = tf.constant([0.0])
  # for i in tf.shape(predictions)[0]:
  #   prediction = predictions[i]
  #   label = labels[i]
  #   mask_pred = tf.not_equal(prediction, tf.zeros_like(prediction))
  #   mask_label = tf.not_equal(label, tf.zeros_like(label))
  #   bleu = tf.add(tf.py_func(compute_bleu, (tf.boolean_mask(labels, mask_label),
  #                                           tf.boolean_mask(predictions, mask_pred)), tf.float32),
  #                 bleu)
  # bleu = tf.divide(bleu, tf.shape(prediction))
